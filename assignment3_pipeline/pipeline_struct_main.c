...
int BTB_I = 0;			// 현재 pc값이 저장된 index 번호를 반환 받음
...
void Fetch(Latch* IF_ID, Latch* ID_EX, Latch* EX_MEM, Count_num* count_num);
...
int BTB_detection(Latch* IF_ID)	// 0일때 무시됨??
{
	...
	for (i = 1; i < 0x100000; i++)
	{
		if (IF_ID->pc == BTB[i][0])
		{
			IF_ID->pc = BTB[i][1];	// BTB기록된 address에 입력
			nPC = IF_ID->pc;		// 기록된 adress를 pc에 입력
...
void Fetch(Latch* IF_ID, Latch* ID_EX, Latch* EX_MEM, Count_num* count_num)
{
	...
	{
		nPC = Mux(Mux(IF_ID->pc + 4, (EX_MEM->BranchAddr + EX_MEM->pc + 4), (EX_MEM->Branch & EX_MEM->ALU_result)), ID_EX->JumpAddr, ID_EX->Jump);
	}	// Jump, Branch에 따른 ->주소값, else ->pc + 4;

	// 기록에 있으면 nPC값 덮어씌우기	
	BTB_I = BTB_detection(IF_ID);

	if (BTB_ex == 1)	// BTB에 기록이 있음
	{
		// BTB_detection();에서 nPC의 값을 불러옴
		if (BTB[BTB_I][2] == 1) // jump의 기록
		{
			EX_MEM->Jump = 0;
		}
		else if (BTB[BTB_I][2] == 2) // Branch not taken
		{
			EX_MEM->Branch = 0;
			count_num->Branch_predict_O++;
		}
		else if (BTB[BTB_I][2] == 3) // Branch taken
		{
			EX_MEM->Branch = 0;
	...
	{
		if (ctrl_dep_Jump > 0)	// Jump = 1
		{
			Latch_Set(IF_ID);
		}
		if (ctrl_dep_Branch > 0)// Branch = 1~3
		{
			Latch_Set(IF_ID);
		}
...
void Decode(Latch* IF_ID, Count_num* count_num)
{
	...
	else if (BTB_ex == 0)
	{
		...
		{
			ctrl_dep_Jump = 1;
			if (IF_ID->RegDst == 1);	// JR제외
			else				        // Jump, JAL
			{
				BTB_Insert(IF_ID->pc, IF_ID->JumpAddr, 1);
			}
		}
	}
...
void Execution(Latch* ID_EX, Count_num* count_num)
{
	...
	else
	{
		...
		else if (BTB_ex == 0)	// 기존의 BTB 기록 X
		{
			...
			{
				ctrl_dep_Branch = 3;
				BTB_Insert(ID_EX->pc, (ID_EX->JumpAddr + ID_EX->pc + 4), 3);
				count_num->Branch_predict_X++;
			}
			else if ((ID_EX->Branch && ID_EX->ALU_result) != 1)
			{
				ctrl_dep_Branch = 0;
				BTB_Insert(ID_EX->pc, ID_EX->pc + 4, 2);
				count_num->Branch_predict_O++;
			}
		}
	}
...
int main(int argc, char* argv[])
{
	...
	while (PC != 0xffffffff)
	{
		Fetch(&latch[0], &latch[3], &latch[5], &count_num);
		Latch_copy(&latch[0], &latch[1]);
		Decode(&latch[2], &count_num);
