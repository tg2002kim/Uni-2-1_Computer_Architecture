...
int jump_slot_config(unsigned int pc_val){
	for(int i=0; i<10; i++){
		if(jump_pc_slot[i] == 0) return 0;	// BTB에 기록X
		else if(jump_pc_slot[i] == pc_val) return 1;//기록O
	}
	return 0;
}
...
void jump_pc_slot_in(unsigned int pc_val, int jump_address){
	for(int i=0; i<10; i++){
		if(jump_pc_slot[i] == pc_val) return ; // BTB에 기록 있음
		else if(jump_pc_slot[i] == 0){			// BTB에 기록X
...
int branch_flag_config(unsigned int pc_val){
	for(int i=0; i<10; i++){
		if(branch_pc_slot[i] == pc_val){	// 현재 pc값과 같음
...
void branch_slot_in(unsigned int pc_val, int branch_address, int taken_val){
	for(int i=0; i<10; i++){
		...
		else if(branch_pc_slot[i] == 0){	// 이전의 Branch 기록 없음
			branch_pc_slot[i] = pc_val;
			branch_target_slot[i] = branch_address;
			if(taken_val == 1){	// Branch 실행
				branch_taken_slot[i][0] = 1;
				branch_taken_slot[i][1] = 1;
			}
			else{				// Branch 실행X
...
void inst_fetch(Latch* IF_ID, Latch* EX_MEM){
	...
	if(branch_flag_config(IF_ID->pc_val) > 1){	// BTB_check / return 2,3 / branch taken
		for(int i=0; i<10; i++){
	...
	if(jump_slot_config(IF_ID->pc_val)){		// BTB_check / return JumpAddr
		for(int i=0; i<10; i++){
...
void execution(Latch* ID_EX, print_num* p_n){
	...
	if((ID_EX->d_i.opcode == 0xc) || (ID_EX->d_i.opcode == 0xd)){
		...
		if(branch_flag_config(ID_EX->pc_val) < 2){	// return 0, 1 / 
			...
			if(ID_EX->c_s.Branch && ID_EX->A_v.ALU_result){	// Branch 실행 = 예측 실패
				p_n->branch_predict_success++;
				branch_slot_in(ID_EX->pc_val, ((ID_EX->d_i.immediate << 2) + ID_EX->pc_val + 4), 1);
