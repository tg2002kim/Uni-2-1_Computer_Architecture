/*

#pragma warning(disable:4996)

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define MAX_LINE_LENGTH 100

int regs[10] = { 0 };
char inst_reg[MAX_LINE_LENGTH];

// µ¡¼À
void add(int op1, int op2) {
    regs[0] = op1 + op2;
    printf("R0: %d = %d + %d\n", regs[0], op1, op2);
}

// »¬¼À
void sub(int op1, int op2) {
    regs[0] = op1 - op2;
    printf("R0: %d = %d - %d\n", regs[0], op1, op2);
}

// °ö¼À
void mul(int op1, int op2) {
    regs[0] = op1 * op2;
    printf("R0: %d = %d * %d\n", regs[0], op1, op2);
}

// ³ª´°¼À
void divide(int op1, int op2) {
    if (op2 == 0) {
        printf("¿À·ù: 0À¸·Î ³ª´­ ¼ö ¾øÀ½\n");
        return;
    }
    regs[0] = op1 / op2;
    printf("R0: %d = %d / %d\n", regs[0], op1, op2);
}

// M ¿¬»ê
void M(int op1, int op2) {
    regs[op2] = op1;
    printf("R%d: %d\n", op2, op1);
}

// C ¿¬»ê (Compare)
void C(int op1, int op2) {
    if (op1 == op2) {
        regs[0] = 0;
    }
    else if (op1 > op2) {
        regs[0] = 1;
    }
    else {
        regs[0] = -1;
    }
    printf("R0: %d\n", regs[0]);
}

// J ¿¬»ê (Jump)
void J(int targetInstruction, FILE* fp) {
    printf("%d\n", targetInstruction);
    fseek(fp, 0, SEEK_SET);

    for (int i = 0; i < targetInstruction; ++i)
        fgets(inst_reg, MAX_LINE_LENGTH, fp);
}

// BEQ ¿¬»ê (Branch_Equal)
void B(int targetInstruction, FILE* fp) {
    if (regs[0] == 0) {
        printf("%d\n", targetInstruction);
        fseek(fp, 0, SEEK_SET);

        for (int i = 0; i < targetInstruction; ++i)
            fgets(inst_reg, MAX_LINE_LENGTH, fp);
    }
}

// GCD ¿¬»ê (Greatest common divisor)
void G(int a, int b) {
    int i, gcd;
    int s = (a < b) ? a : b;
    for (i = 1; i <= s; i++) {
        if ((a % i == 0) && (b % i == 0)) {
            gcd = i;
        }
    }
    regs[0] = gcd;
    printf("R0: %d \n", regs[0]);
}


int main() {
    FILE* fp;

    fp = fopen("input.txt", "r");
    if (fp == NULL) {
        perror("¿À·ù: input.txtÆÄÀÏÀÌ Á¸ÀçÇÏÁö ¾ÊÀ½. \n");
        exit(EXIT_FAILURE);
    }

    while (fgets(inst_reg, MAX_LINE_LENGTH, fp) != NULL) {
        char* op = strtok(inst_reg, " ");
        char* op1_str = strtok(NULL, " ");
        char* op2_str = strtok(NULL, " \n");

        int op1, op2;
        if (op1_str[0] == 'R') {
            op1 = regs[op1_str[1] - '0'];
        }
        else {
            op1 = strtol(op1_str, NULL, 16);
        }

        if (op2_str[0] == 'R') {
            op2 = regs[op2_str[1] - '0'];
        }
        else {
            op2 = strtol(op2_str, NULL, 16);
        }

        if (strcmp(op, "+") == 0) {
            add(op1, op2);
        }
        else if (strcmp(op, "-") == 0) {
            sub(op1, op2);
        }
        else if (strcmp(op, "*") == 0) {
            mul(op1, op2);
        }
        else if (strcmp(op, "/") == 0) {
            divide(op1, op2);
        }
        else if (strcmp(op, "M") == 0) {
            M(op1, op2);
        }
        else if (strcmp(op, "C") == 0) {
            C(op1, op2);
        }
        else if (strcmp(op, "J") == 0) {
            J(op2, fp);
        }
        else if (strcmp(op, "BEQ") == 0) {
            B(op2, fp);
        }
        else if (strcmp(op, "GCD") == 0) {
            G(op1, op2);
        }
    }

    fclose(fp);

    return 0;
}
*/