...
void ctrl(d_instruction* d_i, ctrl_signal* c_s, print_num* p_n, type* t){
	...
	if(((d_i->opcode == 0x0) && ((d_i->funct == 0x8) || (d_i->funct == 0x9))) || (d_i->opcode == 0x2) || (d_i->opcode == 0x3)) 
		c_s->Jump = 1;
	else ;
}
...
void inst_fetch(Latch* IF_ID, Latch* EX_MEM){
	...
	if(ctrl_dependency_count < 1){
		if(ctrl_dependency == 1){
			if(npc == 0xffffffff){
				bubble_generator(IF_ID);
				//print_stage(IF_ID, 1);
				pc = npc;
			...
			IF_ID->d_i.instruction = memory[npc/4];
			npc = npc + 4;
			ctrl_dependency_count++;
		}
		else if(ctrl_dependency == 2) ctrl_dependency_count++;
	}
	else{
		ctrl_dependency_count = 0;
		ctrl_dependency = 0;
	}
	pc = npc;
	//print_stage(IF_ID, 1);
...
void execution(Latch* ID_EX, print_num* p_n){
	...
	if((ID_EX->d_i.opcode == 0xc) || (ID_EX->d_i.opcode == 0xd)){
		...
		Forwarding_unit_in(ID_EX, 0);
		if(ID_EX->c_s.Jump || (ID_EX->c_s.Branch && ID_EX->A_v.ALU_result)){
			ctrl_dependency = 1;
			p_n->branch_predict_fail++;
		}
		else if((ID_EX->c_s.Branch == 1) && (ID_EX->A_v.ALU_result == 0)){
